...
SELECT *
FROM movies 
Right JOIN users on favorite_movie_id = movie_id;
...
SELECT *
FROM movies 
Inner JOIN users on favorite_movie_id = movie_id;


-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 4 >>>>>>>>>>>>>>>>>>>>>>>
-- Determine which lead studio's movies are favorited by users the most.
-- EXPECTED RESULT: Disney
SELECT lead_studio, count(lead_studio) as favorite_studio
FROM movies 
Inner JOIN users on favorite_movie_id = movie_id
Group by lead_studio
Order by favorite_studio desc;
